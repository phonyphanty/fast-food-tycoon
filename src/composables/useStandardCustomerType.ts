...
type GradedPurchase = InstanceType<typeof GradedPurchase>;
...
class StandardCustomerType extends CustomerType {
    private _bestPurchases: Map<string, GradedPurchase[]> = new Map();
    public get bestPurchases(): Map<string, GradedPurchase[]> {
        return this._bestPurchases;
    }
    private set bestPurchases(value: Map<string, GradedPurchase[]>) {
     ...
     * time.
     */
    public findBestPurchases(menu: AbstractMenu, time: AbstractTime): Map<string, GradedPurchase[]> {
        ...
        const purchasesByHour = new Map<string, GradedPurchase[]>();
        ...
        uniqueWTPs.forEach(wtp => {
            ...
            this.WTPByTime.forEach((value, key) => {
                if (value === wtp) {
                    purchasesByHour.set(key, [new GradedPurchase(purchase, 0)]);
                }
            });
        });
        this.bestPurchases = purchasesByHour;
        return purchasesByHour;
    }
    
    public calculateTrafficProbability(time: AbstractTime): number {
